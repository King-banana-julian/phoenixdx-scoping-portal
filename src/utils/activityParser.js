 ...
 * Loads MD files via fetch() from the /public directory —
 * this avoids Rolldown/Vite trying to parse markdown as JavaScript.
 */
...
export const parseActivityMarkdown = (mdContent) => {
  ...
  const phaseStructureMatch =
    mdContent.match(/\*\*Phase structure:\*\*\s*(.+)/i) ||
    mdContent.match(/Phase structure:\s*(.+)/i);
  const phaseStructure = phaseStructureMatch
    ? phaseStructureMatch[1].split(' → ').map((s) => s.trim())
    : [];
  ...
  for (let i = 1; i < sections.length; i++) {
    ...
    if (
      section.startsWith('Activities in this model') ||
      section.trim() === '' ||
      section.startsWith('About this model')
    )
      continue;
    ...
    const parseKVList = (header) => {
      ...
      const content = endMatch
        ? section.slice(start, start + endMatch.index)
        : section.slice(start);
...
// MD files must live under /public so fetch() can reach them at runtime.
const MD_FILES = [
  '/model-a-audit-evaluation.md',
  '/model-b-research-discovery.md',
  '/model-c-design-synthesis.md',
  '/model-d-strategic-facilitation.md',
];
...
export const loadAllActivities = async () => {
  ...
  for (const path of MD_FILES) {
    try {
      ...
      if (!res.ok) {
        console.warn(`Could not load ${path}: ${res.status}`);
        continue;
      }
      const text = await res.text();
      const activities = parseActivityMarkdown(text);
